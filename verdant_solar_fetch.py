...
Configuration (loaded in order; later sources win):
  ...
  3. CLI arguments         — --station / --id / --start / --end etc.
...
Usage:
    python verdant_solar_fetch.py                         # use config
    python verdant_solar_fetch.py --start 2026-07-01 --end 2026-07-05
    python verdant_solar_fetch.py --station XXX --id 11111111111 --start 2026-07-01
...
def fetch_station_day(station_id: str, id_: str, start_date: str, end_date: str,
    ...
    headers = {
        ...
        "Referer": f"{base_url}/device/detail?id={id_}",
        "Origin": base_url,
...
def main():
    ...
    parser.add_argument("--id", default=cfg.get("id"),
                        help="Journey ID")
    ...
    print(f"[*] Fetching station-day data for {station} (id={id_})")
    print(f"    Date range: {start} → {end}")
    print(f"    Endpoint: {args.base_url}/api/vMongoDevice/station-day")

    try:
        data = fetch_station_day(station, id_, start, end, args.base_url)
